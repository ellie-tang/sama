...
INPUT_FACES_DIR = 'inputFaces'  # Directory to monitor for new images
...
class FaceManager:
    ...
    def train_from_folders(self):
        ...
        person_dirs = [d for d in os.listdir(TRAINING_DIR)
                      if os.path.isdir(os.path.join(TRAINING_DIR, d)) and d != 'processed']
    ...
    def process_image(self, image_path, auto_add_unknown=False):
        """Process a single image file to detect and identify faces.

        Args:
            image_path: Path to the image file
            auto_add_unknown: If True, automatically add unknown faces. If False, return them separately.

        Returns:
            tuple: (set of recognized face_ids, list of unknown face embeddings)
        """
        ...
        try:
            ...
            if not os.access(image_path, os.R_OK):
                self.logger.error(f"No read permission for image: {image_path}")
                return set(), []

            # Read image from file with multiple attempts and formats
            frame = None
            max_image_retries = 3

            for retry in range(max_image_retries):
                # Try reading with different OpenCV flags
                read_flags = [cv2.IMREAD_COLOR, cv2.IMREAD_UNCHANGED, cv2.IMREAD_ANYCOLOR]

                for flag in read_flags:
                    try:
                        frame = cv2.imread(image_path, flag)
                        ...
                        self.logger.debug(f"Failed to read image with flag {flag} (attempt {retry+1}): {e}")
                        continue
                ...
                if frame is None:
                    ...
                    try:
                        ...
                        self.logger.info(f"Successfully read image using PIL (attempt {retry+1}): {image_path}")
                        break
                    except Exception as e:
                        self.logger.debug(f"Failed to read image with PIL (attempt {retry+1}): {e}")
            ...
            if frame is None:
                self.logger.error(f"Could not read image after {max_image_retries} attempts: {image_path}")
                return set(), []

            faces = self.face_app.get(frame)

            for face in faces:
                face_id, name, embedding = self._identify_face(face)
                if face_id:
                    # Known face
                    current_faces.add(face_id)
                    display_text = name if name else (face_id[:8] + "...")
                    print(f"Detected face: {display_text}")
                else:
                    # Unknown face
                    if auto_add_unknown:
                        # Old behavior: automatically add
                        new_face_id = self.register_new_face(embedding, '')
                        current_faces.add(new_face_id)
                        print(f"Detected face: {new_face_id[:8]}...")
                    else:
                        # New behavior: collect for user confirmation
            ...
            return current_faces, unknown_embeddings
        except Exception as e:
            self.logger.error(f"Error processing image {image_path}: {e}")
            return set(), []

    def _identify_face(self, face):
        """Identify a face. Returns (face_id, name, embedding) or (None, None, embedding) if unknown."""
        ...
        # Unknown face - return None for face_id and name, but include embedding
        return None, None, embedding

    def register_new_face(self, embedding, name=''):
        """Register a new face with the given embedding and optional name.
        ...
        Returns:
            face_id: The ID of the newly registered face
        """
        new_face_id = str(uuid.uuid4())
        self.known_faces[new_face_id] = {
            'embedding': embedding,
        ...
        if name:
            self.face_summaries[new_face_id] = {
                'summary': f"This is {name}. No additional information available yet.",
                'last_updated': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'message_count': 0,
                'name': name
            }
        ...
        self.save_known_faces()
        self.logger.info(f"Registered new face: {name or new_face_id[:8]}")
        return new_face_id
    ...
    def delete_face(self, face_id):
        ...
        try:
            ...
            name = self.get_name_for_face(face_id) or f"Face {face_id[:8]}"

            # Remove from known faces
            del self.known_faces[face_id]

            # Remove from active faces if present
            with self.lock:
                self.active_faces.discard(face_id)

            # Remove from memories
            if face_id in self.face_memories:
                del self.face_memories[face_id]

            # Remove from message counts
            if face_id in self.face_message_counts:
                del self.face_message_counts[face_id]

            # Remove from summaries
            if face_id in self.face_summaries:
                del self.face_summaries[face_id]

            # Remove from CSV file
            self._delete_face_from_summary_csv(face_id)

            # Save updated known faces
            self.save_known_faces()

            self.logger.info(f"Deleted face: {name} ({face_id[:8]})")
            return True
        except Exception as e:
            self.logger.error(f"Error deleting face: {e}")
            return False

    def _delete_face_from_summary_csv(self, face_id):
        """Remove a face from the summary CSV file."""
        if not os.path.exists(SUMMARY_FILE):
            return
        ...
        try:
            ...
            with open(SUMMARY_FILE, 'r', newline='', encoding='utf-8', errors='ignore') as file:
                reader = csv.reader(file)
                rows = [row for row in reader if not (len(row) > 0 and row[0] == face_id)]

            with open(SUMMARY_FILE, 'w', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerows(rows)
        except Exception as e:
            self.logger.error(f"Error removing face from summary CSV: {e}")
...
class ImageFileHandler(FileSystemEventHandler):
    """Handler for monitoring new images in inputFaces directory."""
    def __init__(self, face_manager, chat_app):
        self.face_manager = face_manager
        self.chat_app = chat_app
        self.logger = logging.getLogger(self.__class__.__name__)
        self.processed_files = {}  # Track processed files with timestamps to avoid duplicates

    def on_created(self, event):
        self._process_file_event(event, "created")

    def on_modified(self, event):
        # Also handle modified events for files that are being written
        if not event.is_directory:
            self._process_file_event(event, "modified")

    def _process_file_event(self, event, event_type):
        if event.is_directory:
            return

        # Clean up old processed files (older than 1 hour)
        current_time = time.time()
        expired_files = [f for f, t in self.processed_files.items() if current_time - t > 3600]
        for expired_file in expired_files:
            del self.processed_files[expired_file]

        # Validate file path is within expected directory
        try:
            real_path = os.path.realpath(event.src_path)
            expected_dir = os.path.realpath(INPUT_FACES_DIR)
            if not real_path.startswith(expected_dir):
                self.logger.warning(f"File outside monitored directory: {event.src_path}")
                return
        except Exception as e:
            self.logger.error(f"Error validating file path {event.src_path}: {e}")
            return

        # Prevent duplicate processing (ignore events within 5 seconds of successful processing)
        current_time = time.time()
        if event.src_path in self.processed_files:
            last_processed = self.processed_files[event.src_path]
            if current_time - last_processed < 5.0:  # 5 second cooldown
                return

        # Check if it's an image file (case insensitive)
        if event.src_path.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp')):
            # Wait for file write completion with enhanced retry mechanism
            max_retries = 10
            last_size = 0
            stable_count = 0
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # File is stable for at least 2 checks
                    if stable_count >= 2:
                        # Final test: try to open and read a bit
                        with open(event.src_path, 'rb') as f:
                            f.read(min(1024, current_size))  # Read up to 1KB or file size
                        break
                    ...
                    if attempt < max_retries - 1:
                        ...
                        self.logger.error(f"File not ready after {max_retries} attempts: {event.src_path} - {e}")
                        return

            try:
                print(f"\nNew image detected ({event_type}): {event.src_path}")
                print(f"File size: {os.path.getsize(event.src_path)} bytes")

                # Add extra wait for image file format to settle
                time.sleep(0.5)

                detected_faces, unknown_embeddings = self.face_manager.process_image(event.src_path, auto_add_unknown=False)

                total_faces = len(detected_faces) + len(unknown_embeddings)

                if total_faces > 0:
                    print(f"Found {total_faces} face(s) in the image")

                    # Handle known faces
                    if detected_faces:
                        active_faces_info = self.face_manager.get_active_faces_info()
                        if active_faces_info:
                            names = [face['name'] for face in active_faces_info]
                            print(f"Known faces: {', '.join(names)}")

                    # Handle unknown faces - ask user
                    if unknown_embeddings:
                        print(f"\n{len(unknown_embeddings)} unknown face(s) detected!")
                        for i, embedding in enumerate(unknown_embeddings, 1):
                            print(f"\n--- Unknown Face #{i} ---")
                            response = input(f"Add this face to the system? (yes/no): ").strip().lower()

                            if response in ['yes', 'y']:
                                name = input("Enter name for this person (or press Enter to skip): ").strip()
                                face_id = self.face_manager.register_new_face(embedding, name)
                                if name:
                                    print(f"✓ Added: {name}")
                                else:
                                    print(f"✓ Added with ID: {face_id[:8]}...")
                            else:
                                print("✗ Skipped - face not added to system")
                else:
                    print("No faces detected in the image")

                # Keep the processed image (do not delete)
                print(f"Processing complete - image preserved: {event.src_path}")

                # Only mark as processed if we successfully detected faces
                if detected_faces or unknown_embeddings:
                    self.processed_files[event.src_path] = current_time
                else:
                    # If no faces detected, don't mark as processed - might be a timing issue
                    pass

            except Exception as e:
                self.logger.error(f"Error processing new image {event.src_path}: {e}")
                # Keep the file even if processing failed (do not delete)
                print(f"Processing failed but image preserved: {event.src_path}")
                # Don't mark as processed to allow retry later
                pass
...
class ChatApp:
    ...
    def start_monitoring(self):
        """Start monitoring the inputFaces directory for new images."""
        try:
            # Ensure directory exists and is accessible
            if not os.path.exists(INPUT_FACES_DIR):
                self.logger.error(f"Input directory does not exist: {INPUT_FACES_DIR}")
                return False

            if not os.access(INPUT_FACES_DIR, os.R_OK):
                self.logger.error(f"No read access to directory: {INPUT_FACES_DIR}")
                return False

            self.file_handler = ImageFileHandler(self.face_manager, self)
            self.observer = Observer()
            self.observer.schedule(self.file_handler, INPUT_FACES_DIR, recursive=False)
            self.observer.start()
            print(f"Started monitoring {INPUT_FACES_DIR} for new images...")
            return True
        except Exception as e:
    ...
    def stop_monitoring(self):
        """Stop monitoring the inputFaces directory."""
        if self.observer:
            self.observer.stop()
            ...
            print("Stopped monitoring inputFaces directory")
    ...
    def run(self):
        ...
        print("  /delete  - Delete a face from the system")
        print("  /faces   - Show all known faces")
        print("  /summary - Show summaries for active faces")
        print("\nPlace new images in the 'inputFaces' directory for face recognition")
        print("Images will be automatically processed and deleted after recognition")

        if not self.start_monitoring():
            print("ERROR: Failed to start directory monitoring. Exiting.")
            return
                ...
                if user_message.lower() == '/quit':
                    ...
                    if not face_ids:
                        ...
                        try:
                            ...
                            if 1 <= choice <= len(face_ids):
                                selected_id = face_ids[choice-1]
                                selected_name = self.face_manager.get_name_for_face(selected_id) or f"Face {selected_id[:8]}"
                                confirm = input(f"Delete '{selected_name}'? (yes/no): ")
                                if confirm.lower() in ['yes', 'y']:
                                    if self.face_manager.delete_face(selected_id):
